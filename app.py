...
# --- DB SETUP - WORKS WITH OLD + NEW DB ---
...
# --- GROQ CLIENT WITH CHATGPT MODEL ---
client = Groq(api_key=st.secrets["GROQ_API_KEY"])

def get_ai_response(messages):
    try:
        resp = client.chat.completions.create(
            model="openai/gpt-oss-120b", # CHATGPT STYLE MODEL FROM GROQ
            messages=messages,
            temperature=0.7,
            max_tokens=1024
        )
...
# --- MAIN APP ---
    ...
    # 2. IMAGE GEN - 3 FREE TRIALS
        ...
        if st.session_state.is_pro == 0 and st.session_state.img_trials <= 0:
            ...
            prompt = st.text_input("Describe the image you want", placeholder="A futuristic city at night")
            if st.button("Generate Image", type="primary"):
                with st.spinner("Generating in 5s..."):
